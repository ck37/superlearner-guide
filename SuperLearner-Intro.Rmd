...
* Installing
* Create dataset
* Fit single models: lasso, randomForest, GBM
* Fit ensemble
* Customize a model parameter.
* Fit a caret model.
* Fit ensemble with external cross-validation.
* Plot external cross-validation results.
...
Let's fit 2 separate models: lasso (sparse, penalized OLS) and randomForest.
...
# (Not strictly necessary, but pretending that we have another dataset to check.)
# onlySL is specified so we don't fit algorithms that had weight = 0.
...
AUC goes from 0.5 (terrible) to 1.0 (perfect). So we are looking pretty good!
...
Let's make a variant of RandomForest that fits more trees, which should increase our accuracy.
...
For randomForest there are two hyperparameters we want to explore: mtry and maximum leaf nodes. Mtry is how many features are randomly chosen within each decision tree node. Maximum leaf nodes controls how complex each tree can get.
...
We see here that mtry = 14 performed a little bit better than mtry = 3 or mtry = 7, although the difference is not significant. If we used more data and more cross-validation folds we might see more significant differences. A higher mtry does better when a small percentage of variables are predictive of the outcome, because it gives each tree a better chance of finding a useful variable.

Note that SL.randomForest and SL.randomForest_2 have the same settings, and their performance is very similar - statistically a tie. It's not exactly the same due to random variation in the two forests.

## Multicore parallelization

SuperLearner makes it easy to use multiple CPU cores on your computer to speed up the calculations. We need to tell R to use multiple CPUs, then change our function to mcSuperLearner.
...
If we want to use multiple cores for normal SuperLearner, not CV.SuperLearner, we need to change the function name to `mcSuperLearner`.
...
XGBoost is a version of GBM that is even faster. GBM's adaptivity is determined by its configuration, so we want to thoroughly test a wide range of configurations for any given problem. Let's do 60 now. This will take a good amount of time (~7 minutes on my computer) so we need to at least use multiple cores, if not multiple computers.

```{r cache=T, fig.height=8}
# 4 * 4 * 3 = 60 different configurations.
...
* Multi-node parallel SuperLearner
* Cumulative library analysis
* Individual-algorithm library analysis
...
* Applied Predictive Modeling by MAx
